...
- `ENTRYPOINT`: 设置容器启动时运行的命令
  - 让程序以应用程序或者服务的形式运行
  - 如果`docker run`指定了其他参数，`ENTRYPOINT`不运行
...
```docker
FROM python:3.8.5
LABEL maintainer="vip.gewei@foxmail.com"
...
# build成功之后
sudo docker images
sudo doker run alphagrey/simple-flask
sudo doker run -d alphagrey/simple-flask # 后台运行
```
